...
var hero = new Character(cellSize, heroReset.x, heroReset.y, heroReset.speed, pixCharacter);
...
function update(mod) {
  ...
  /// BARRIERS ///
  barrier1.setX(barrier1.getX() - barrier1.getSpeed() * mod);
  if (barrier1.getX() <= -100) {
    barrier1.setX(barrierReset.x);
  }
...
function drawScene() {
  ...
  // draw a character image depending on the runPosition
  var currentImage = function(obj, nr) {
    ctx.drawImage(obj.getImage(nr), obj.getX(), obj.getY());
  };
...
var time = Date.now();
var gameLoop = setInterval(run, 20);
